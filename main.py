# Based on original work by: Copyright (c) 2026 KOTIN888 (MIT License)
# Modded for Spotify Desktop integration
...
def get_token() -> str:
    ...
    if not token:
        log.error("Token tidak ditemukan! Masukkan token Anda di config.json atau environment variable DISCORD_TOKEN")
        sys.exit(1)
    return token
...
async def get_smtc_state() -> Optional[dict]:
    try:
        ...
        status = int(spotify_session.get_playback_info().playback_status)
        playing = status == 4
        title = info.title or ""
        if not title:
            return None
...
async def get_lyrics(http_session: aiohttp.ClientSession, artist: str, title: str) -> list[tuple[float, str]]:
    ...
    if cached:
        if cached.get("expires", 0) > time.time():
            return cached["lines"]
    log.info(f"Mencari lirik: {artist} — {title}")
    queries = [
        f"{artist} {title}",
        title,
    ...
    for q in queries:
        ...
        if lines:
            log.info(f"lrclib: Berhasil memuat {len(lines)} baris lirik")
            break
    if not lines:
        log.warning("Lirik tidak ditemukan di lrclib")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.patch(f"{self.API}/users/@me/settings", json=payload, headers=self.headers) as r:
                    if r.status == 200:
                        log.info(f"Status Discord Berubah → {text!r}")
                    elif r.status == 401:
                        log.error("Token Discord Salah! Periksa file config.json")
                        ...
                        log.warning(f"API Discord Error: {r.status}")
        except Exception as e:
...
async def main():
    ...
    log.info("Program Berjalan — Menunggu musik dari Spotify...")
            ...
            try:
                ...
                if state is None:
                    ...
                    if none_count >= MAX_NONE_COUNT and current_key is not None:
                        log.info("Spotify ditutup — Membersihkan status Discord")
                        current_key = None
                        lines = []
                        none_count = 0
                        prev_position = -999.0
                    ...
                    if key != current_key:
                        log.info(f"Lagu Baru Terdeteksi: {artist} — {title}")
                        current_key = key
                        prev_position = -999.0
                    ...
                    if not playing:
                        if current_key is not None:
                            log.info("Musik di-pause — Membersihkan status Discord")
                            current_key = None
                            lines = []
                            prev_position = -999.0
                            await dc.clear_status()
                ...
                log.error(f"Error di dalam loop: {e}")
            await asyncio.sleep(2)
    
    log.info("Program dihentikan.")
